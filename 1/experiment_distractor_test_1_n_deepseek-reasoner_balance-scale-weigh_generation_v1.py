...
class HeatMilkForBabyGame(TextGame):
    ...
    def generatePossibleActions(self):
        ...
        # Precompute thermometer object if available
        thermometer_obj = None
        if "thermometer" in allObjects:
            thermometer_obj = allObjects["thermometer"][0]
        ...
        for objName, objs in allObjects.items():
            for obj in objs:
                ...
                # Use thermometer on object (only if thermometer exists)
                if thermometer_obj is not None and objName != "thermometer":
                    self.addAction(f"use thermometer on {objName}", ["use", thermometer_obj, obj])

...
class TakePhotoGame(TextGame):
    ...
    def generatePossibleActions(self):
        ...
        # (1-arg) Eat, Take, Open/Close, Detailed look/examine, Detailed focus, Turn on/Turn off device
    ...
    def step(self, actionStr):
        ...
        actions = {
            ...
            "press shutter": lambda: self.actionPressShutter(),  # take a picture
            ...
            "use": lambda: self.actionUse(action[1], action[2]),  # OMIT, UNUSED (boiling water)
            "focus": lambda: self.actionFocus(action[1]),  # focus the camera on an object
            "rotate": lambda: self.actionRotate(action[1], action[2])  # rotate a dial on the camera
        }
...
if __name__ == "__main__":
    ...
    main(TakePhotoGame(randomSeed=0))
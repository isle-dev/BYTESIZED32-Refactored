...
def main():
    ...
    for n, row in experiment_df.iterrows():
        ...
        for key in ablation_games:
            ...
            prompt += "```\n"\
                      "GameBasic.py includes the classes GameObject, Container, Device, Substance, World, Agent, and a base implementation of TextGame. In your game code, you must derive new classes from these basic classes to build your game logic.\n"
            prompt += "Your code must contain a derived class of TextGame that has the following member functions: __init__(self, randomSeed), getTaskDescription(self), generatePossibleActions(self), step(self, actionStr), calculateScore(self).\n"
